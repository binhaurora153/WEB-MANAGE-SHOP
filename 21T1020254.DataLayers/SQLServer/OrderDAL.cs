    ...
    public class OrderDAL : BaseDAL, IOrderDAL
    {
...

        {
            ...
            using (var connection = OpenConnection())
            {
                ...
                list = connection.Query<Order>(sql: sql, param: parameters, commandType: CommandType.Text).AsList();
            }
            return list;
        }
        ...
        public IList<OrderDetail> ListDetails(int orderID)
        {
            ...
            using (var connection = OpenConnection())
            {
                ...
                var parameters = new { OrderID = orderID };
                list = connection.Query<OrderDetail>(sql: sql, param: parameters, commandType: CommandType.Text).AsList();
            }
            return list;
        }
        ...
        public IList<Order> ListOrderCustomers(int page = 1, int pageSize = 0,
                         int status = 0, DateTime? fromTime = null, DateTime? toTime = null,
                         string searchValue = "", int? customerId = null)  // Thêm tham số customerId
        {
            List<Order> list = new List<Order>();
            ...
            using (var connection = OpenConnection())
            {
                var sql = @"with cte as
                    ...
                    where (@Status = 0 or o.Status = @Status)
                     ...
                     and (@SearchValue = N'' 
                         ...
                         or e.FullName like @SearchValue
                         or s.ShipperName like @SearchValue)
                     and (@CustomerID is null or o.CustomerID = @CustomerID)  -- Thêm điều kiện lọc theo CustomerID
                    )
                    select * from cte
                    where (@PageSize = 0)
                      or (RowNumber between (@Page - 1) * @PageSize + 1 and @Page * @PageSize)
                ...
                var parameters = new
                {
                    ...
                    Page = page,
                    PageSize = pageSize,
                    CustomerID = customerId  // Thêm tham số CustomerID vào
                };
        ...
        public int CountOrderCustomer(int status = 0, DateTime? fromTime = null, DateTime? toTime = null, string searchValue = "", int? customerId = null)
        {
            int count = 0;
            if (!string.IsNullOrEmpty(searchValue))
                searchValue = "%" + searchValue + "%";
